...
// Use filteredWorks instead of works in your JSX rendering
  ...
  .filter((work) => {
    const matchesStartDate = !startDateFilter || new Date(work.start) >= new Date(startDateFilter);
    return matchesStartDate;
  })
  .filter((work) => {
    const matchesEndDate = !endDateFilter || new Date(work.end) <= new Date(endDateFilter);
