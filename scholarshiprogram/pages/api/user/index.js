...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      console.log('Fetching data from the "user" collection...');
      const docs = await UserModel.find();
      console.log('Fetched data from the "user" collection:', docs);
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        try {
          ...
          if (typeof fields === 'object' && !Array.isArray(fields)) {
            ...
            console.log('Inserted data into the "user" collection:', doc);
